...
- 🕐 **Janela de deploy: CANCELADA pelo dono (09/09/2026).** A ordem agora é **mergear NA HORA e sem perguntar**, inclusive mudança de `backend/` em horário de loja aberta. Não segurar PR pronto esperando almoço/noite, e não perguntar "subo agora ou na janela?" — terminou e verificou, sobe. O custo continua existindo e é bom saber qual é: o deploy do Railway é COM downtime (~40-60s — o volume anexado impede overlap) e esfria os caches (catálogo remonta em ~17s, espelho frio); foi isso que gerou a reclamação "sistema travando muito" das lojas em 01/09 (3 deploys entre 09:07 e 10:15). Só que a decisão do dono é que atraso de correção custa mais que o restart. Onde ainda vale economizar restart sem atrasar nada: **vários PRs prontos ao mesmo tempo mergeiam em sequência** (1 restart em vez de N), e commit de docs/frontend/scripts **não reinicia o backend** de todo jeito (railway.json tem `watchPatterns: ["backend/**", "railway.json"]` desde 01/09; o Vercel já tinha `ignoreCommand`). O que NÃO mudou: verificar antes (build + boot-test + SQL) continua obrigatório — a pressa é no merge, não na conferência.
