...
export const CAMPANHA_PADRAO: ConfigCampanha = {
  ...
  nome: 'Inverno',
  pct: 30,
  termos: ['CASACO', 'CALÇA MOLETOM', 'JAQUETA', 'INVERNO'],
};
...
/** null = termo que não pega nada (vazio, só símbolo, `*` curto demais). */
...
export function normalizarConfig(c: Partial<ConfigCampanha> | null | undefined): ConfigCampanha {
  ...
  const termos: string[] = [];
  for (const t of Array.isArray(base.termos) ? base.termos : []) {
    const comp = compilarTermo(t);
    if (!comp) continue;
    const k = comp.palavras.map((p) => `${p.raiz}${p.prefixo ? '*' : ''}`).join(' ');
  ...
  return {
    ...
    termos: termos.slice(0, TERMOS_MAX),
  };
}
...
export interface RegraCampanha {
  ...
  rotulo: string;
  /** Muda quando config ou exceções mudam — a vitrine remonta por ela. */
  assinatura: string;
  /**
...
export function criarRegra(
  ...
  const termos = config.termos
    ...
    .filter((t): t is TermoCompilado => !!t);
  ...
  const decidir = (linha: LinhaCampanha, textoPronto?: TextoIndexado): DecisaoCampanha => {
    ...
    if (!config.ativa) {
      return { entra: false, motivo: 'campanha desligada', termo: null, excecao: null, chave };
    }
    const excecao = chave ? porChave.get(chave) ?? null : null;
    if (excecao?.decisao === 'fora') {
      return { entra: false, motivo: 'tirada da campanha na mão', termo: null, excecao, chave };
    }
    if (excecao?.decisao === 'dentro') {
      return { entra: true, motivo: 'incluída na campanha na mão', termo: null, excecao, chave };
    }
    const texto = textoPronto ?? textoDaLinha(linha);
    const casou = termos.find((t) => termoCasa(t, texto));
    if (casou) {
      return { entra: true, motivo: `casou com "${casou.original}"`, termo: casou.original, excecao: null, chave };
    }
    return { entra: false, motivo: 'nenhum termo da campanha na descrição', termo: null, excecao: null, chave };
  };
