...
export class ContasPagarService {
  ...
  async funcionariasResumo(mes?: string) {
    ...
    type Pessoa = { nome: string; cpf: string | null; sellerId: string | null; totalCents: number; saldoAdiantamentoCents: number; itens: any[] };
    ...
    for (const r of rows) {
      ...
      if (!p) porPessoa.set(key, (p = { nome: r.sellerNome || '?', cpf: r.sellerCpf || null, sellerId: r.sellerId, totalCents: 0, saldoAdiantamentoCents: 0, itens: [] }));
    ...
    for (const s of saldos) {
      ...
      if (s.cents > 0 && !porPessoa.has(key)) {
        porPessoa.set(key, { nome: s.sellerNome, cpf: null, sellerId: s.sellerId, totalCents: 0, saldoAdiantamentoCents: s.cents, itens: [] });
      }
    }
