...
export class DceEmitService {
  ...
  private buildXml(p: {
    ...
    return (
      ...
      `</dest>` +
      det +
      // O grupo do total chama-se <total> (não <tot> — o validador recusou
      // tot E transp depois do det; o exemplo do manual usava <total>).
      `<total><vDC>${this.money(p.valorTotal)}</vDC></total>` +
