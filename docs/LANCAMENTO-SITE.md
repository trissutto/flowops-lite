...
85. ✅ FEITO (12/08) — Vale-troca de ponta a ponta. Só nasce depois da conferência aprovada com **checklist obrigatório**; entra em `site_cupons` (fixed, uso único, 90 dias) e é **nominal no CPF** — código vazado num print não vira compra de outra pessoa. Vale **no site E na loja**: o PDV resolve o código em `site_cupons` quando não acha em `pdv_returns`, dá a baixa no mesmo contador `usos` que o checkout usa (é o que impede gastar nos dois canais) e devolve o crédito se a venda for cancelada. Uso parcial continua gerando vale residual
