...
Busca `GET /api/checkout/:id` → `{ ok, order }` e exibe:

1. **Chip de status** — "Pagamento confirmado" (pago) ou "Aguardando
   pagamento" (PIX pendente/boleto), com o tom certo em cada um.
2. **Número do pedido** em destaque (`LP-000123`, display serif) — é o dado
   que a cliente anota/manda no WhatsApp.
...
do servidor: o webhook de pagamento confirma, grava `paid` e dispara com
dedupe por `transaction_id` (docs/purchase.md). Thank you que dispara purchase
