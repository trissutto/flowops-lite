...
/**
 * FOTO DA AVALIAÇÃO — repassa o arquivo pro backend, que grava no R2.
 *
...
export async function POST(request: Request) {
  ...
  if (!token) return NextResponse.json({ erro: 'sem sessao' }, { status: 401 });

  const base = process.env.FLOWOPS_API_URL?.replace(/\/$/, '');
  if (!base) {
    console.error('[conta] foto avaliacao: FLOWOPS_API_URL não configurada');
  ...
  try {
    ...
    const res = await fetch(`${base}/customers/app/avaliacoes/foto`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
      body: corpo,
      cache: 'no-store',
      // Foto de celular sobe devagar: o timeout curto do helper padrão
      // derrubaria upload que ia dar certo.
    ...
    console.error('[conta] foto avaliacao:', e);
