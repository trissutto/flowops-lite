            ...
            <tbody>{customers.map((customer) => {
              ...
              return <Fragment key={customer.id}>
                <tr onClick={() => openCustomer(customer.id)} className="cursor-pointer border-t border-[#ECEBE7] text-sm hover:bg-[#F8F8F5]">
                  ...
                  <td className="px-3 py-3.5"><div className="flex items-center gap-1">{isAdmin && customer.origins.length === 1 && customer.orderCount === 0 && Number(customer.ltvCents) === 0 && <button onClick={(event) => { event.stopPropagation(); setDeleteOrigin(customer.origins[0]); }} title="Excluir linha duplicada vazia" className="rounded-md p-1.5 text-[#9A342B] hover:bg-[#FFF2F0]"><Trash2 className="h-4 w-4" /></button>}<button onClick={(event) => { event.stopPropagation(); setExpandedId(expanded ? null : customer.id); }} title="Ver cadastros vinculados" className="rounded-md p-1.5 hover:bg-[#EDECE7]"><ChevronDown className={`h-4 w-4 transition-transform ${expanded ? 'rotate-180' : ''}`} /></button></div></td>
                </tr>
