...
export default function FichaClienteBetaPage() {
  ...
  const channels = useMemo(() => {
    ...
    return raw.filter(Boolean).map((v: string) => origemLabel[v] || v);
  }, [person, detail]);
    ...
    <main className="min-h-screen bg-[#F7F7F5] text-[#27241F]">
      ...
      <div className="mx-auto max-w-7xl space-y-4 px-4 py-6 sm:px-6">
        {tab === 'resumo' && <>
          ...
          <div className="grid gap-4 lg:grid-cols-2"><Card><SectionTitle>Contato e relacionamento</SectionTitle><div className="grid grid-cols-2 gap-4"><Field label="WhatsApp" value={phone(detail.whatsapp)} /><Field label="E-mail" value={detail.email} /><Field label="Vendedora que captou" value={detail.originSeller} /><Field label="Última compra" value={date(detail.lastOrderAt)} /></div></Card><Card><SectionTitle>Origem da cliente</SectionTitle><div className="space-y-3 text-sm"><div className="flex gap-3"><Store className="h-4 w-4 text-[#8C7325]" /><span>{detail.originStore?.name || 'Loja não informada'}</span></div><div className="flex gap-3"><UserRound className="h-4 w-4 text-[#8C7325]" /><span>{channels.join(', ') || 'Origem não informada'}</span></div><div className="flex gap-3"><Clock3 className="h-4 w-4 text-[#8C7325]" /><span>Atualizada em {date(detail.updatedAt, true)}</span></div></div></Card></div>
        </>}
