...
function flowops_unpack_other_fields($raw) {
    ...
    // Tenta JSON primeiro, depois PHP serialized (CartFlows usa os 2 formatos
    // dependendo da versão + do tipo de checkout: WooCommerce padrão vs CartFlows).
    ...
    /**
     * Pick helper — procura a primeira chave existente e não-vazia entre várias
     * variantes. Cart Abandonment Recovery salva com nomes diferentes conforme o
     * checkout: CartFlows usa `wcf_*`, checkout WC padrão usa `billing_*`, e
     * também aparecem versões "flat" (first_name, last_name).
     */
    $pick = function (array $keys) use ($obj) {
        foreach ($keys as $k) {
            if (isset($obj[$k]) && $obj[$k] !== '') return $obj[$k];
        }
        return null;
    };

    $out = [
        // Mantém os nomes originais do plugin pra não quebrar o backend atual.
        'wcf_first_name'      => $pick(['wcf_first_name', 'billing_first_name', 'first_name', 'shipping_first_name']),
        'wcf_last_name'       => $pick(['wcf_last_name', 'billing_last_name', 'last_name', 'shipping_last_name']),
        'wcf_phone_number'    => $pick(['wcf_phone_number', 'billing_phone', 'phone', 'billing_phone_number']),
        'wcf_billing_company' => $pick(['wcf_billing_company', 'billing_company', 'company']),
        'wcf_location'        => $pick(['wcf_location', 'location']),
        'wcf_billing_address_1' => $pick(['wcf_billing_address_1', 'billing_address_1', 'address_1']),
        'wcf_billing_address_2' => $pick(['wcf_billing_address_2', 'billing_address_2', 'address_2']),
        'wcf_billing_city'    => $pick(['wcf_billing_city', 'billing_city', 'city']),
        'wcf_billing_state'   => $pick(['wcf_billing_state', 'billing_state', 'state']),
        'wcf_billing_postcode'=> $pick(['wcf_billing_postcode', 'billing_postcode', 'postcode']),
        'wcf_billing_country' => $pick(['wcf_billing_country', 'billing_country', 'country']),
        'wcf_order_bump_products' => $pick(['wcf_order_bump_products']),
        'wcf_user_id'         => $pick(['wcf_user_id', 'user_id']),
    ];

    // Remove nulls pra ficar limpo no JSON final.
    $out = array_filter($out, fn($v) => $v !== null);

    // Bruto pra debug (só as chaves — não o valor inteiro, pra não inflar resposta).
    $out['_keys'] = array_keys($obj);
